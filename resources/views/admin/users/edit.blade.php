<x-admin.layout>
    ...
    <div class="main-content-wrap">
        ...
        <!-- new-category -->
        <div class="wg-box">
            <form class="form-new-product form-style-1" action="{{ route('admin.users.update', $user) }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                @method('put')
                <fieldset class="userName">
                    <div class="body-title">أسم المستخدم<span class="tf-color-1">*</span></div>
                    <x-admin.forms.input name="name" :object="$user" />
                </fieldset>

                <fieldset class="email">
                    <div class="body-title">البريد الإلكتروني<span class="tf-color-1">*</span></div>
                    <x-admin.forms.input name="email" :object="$user" />
                </fieldset>
                        ...
                        <select class="" name="role_id" style="width: 235.5px">
                            ...
                            @foreach ($roles as $role)
                                <option value="{{ $role->id }}"
                                    {{ $role->id == ($user->roles->first()->id ?? "")  ? 'selected' : '' }}>{{ $role->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
