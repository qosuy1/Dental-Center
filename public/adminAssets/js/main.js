...
//for upload files
function previewFile(fieldNumber) {
    const fileInput = document.getElementById(`file-${fieldNumber}`);
    const previewContainer = document.getElementById(
        `file-preview-${fieldNumber}`
    );
    const previewImage = document.getElementById(
        `preview-image-${fieldNumber}`
    );
    const fileInfo = document.getElementById(`file-info-${fieldNumber}`);
    const uploadText = document.getElementById(`upload-text-${fieldNumber}`);
    const uploadLabel = fileInput.closest(".uploadfile");
    ...
    if (fileInput.files && fileInput.files[0]) {
        ...
        uploadText.innerHTML = `File selected: <span class="tf-color">${file.name}</span>`;
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".item.up-load").forEach((uploadArea) => {
        ...
        uploadArea.addEventListener("drop", (e) => {
            e.preventDefault();
            uploadArea.classList.remove("dragover");
            const fieldNumber = e.currentTarget.id.replace(
                "upload-container-",
                ""
            );
            document.getElementById(`file-${fieldNumber}`).files =
                e.dataTransfer.files;
            previewFile(fieldNumber);
        });
    });
});

// Bootstrap JS + JavaScript logic (toggleSwitch)
